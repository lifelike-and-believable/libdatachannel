...
shared_ptr<rtc::PeerConnection> createPeerConnection(const rtc::Configuration &config,
	...
	pc->onLocalDescription([wws, id](rtc::Description description) {
		std::cout << "[->] Sending local description to " << id << " (type: " << description.typeString() << ")" << std::endl;
		json message = {{"id", id},
	...
	pc->onDataChannel([id](shared_ptr<rtc::DataChannel> dc) {
		std::cout << "[+] DataChannel received from " << id << " with label: \"" << dc->label() << "\"" << std::endl;

		dc->onOpen([id, wdc = make_weak_ptr(dc)]() {
			std::cout << "[✓] DataChannel opened from " << id << std::endl;
		});

		dc->onClosed([id]() {
			std::cout << "[!] DataChannel closed from " << id << std::endl;
		});

		dc->onMessage([id](auto data) {
			if (std::holds_alternative<std::string>(data)) {
				std::cout << "[✓] Message received from " << id << ": \"" << std::get<std::string>(data) << "\"" << std::endl;
			} else {
				std::cout << "[✓] Binary message received from " << id 
				          << " (size: " << std::get<rtc::binary>(data).size() << " bytes)" << std::endl;
			}
		});
	...
	pc->onTrack([id](shared_ptr<rtc::Track> track) {
		...
		track->onClosed([id]() {
			std::cout << "[!] Audio track closed from " << id << std::endl;
		});

		// Buffer to accumulate audio data for playback
		static std::vector<std::byte> audioBuffer;
		static bool playbackStarted = false;

		track->onMessage([id](auto data) {
			if (std::holds_alternative<rtc::binary>(data)) {
				auto &binary = std::get<rtc::binary>(data);
				std::cout << "[✓] Audio data received from " << id 
				          << " (size: " << binary.size() << " bytes)" << std::endl;
				
				// Extract PCM data from RTP packet (skip 12-byte RTP header)
				if (binary.size() > 12) {
					audioBuffer.insert(audioBuffer.end(), binary.begin() + 12, binary.end());
					
					// When we have accumulated enough data, play it through speakers
					if (!playbackStarted && audioBuffer.size() > 48000) { // ~0.5 seconds at 48kHz 16-bit
						playbackStarted = true;
						std::cout << "[*] Playing audio through speakers (aplay/paplay)..." << std::endl;
						
						// Write audio data to a file and play it
						...
						std::string playCmd = "aplay -f S16_LE -r 48000 -c 1 " + tempFile + " 2>/dev/null || "
						                     "paplay --raw --format=s16le --rate=48000 --channels=1 " + tempFile + " 2>/dev/null || "
						                     "ffplay -f s16le -ar 48000 -ac 1 -nodisp -autoexit " + tempFile + " 2>/dev/null &";
